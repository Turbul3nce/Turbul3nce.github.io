...
##### Payload Breakdown:
Using the Gopher protocol through the SSRF, we can interact with uWSGI on its internal port (5000). Allowing us to achieve some cool stuff. In this case, we’re looking at RCE. Basically, if we can get Python code inside a file (we can, the JSON file) we can use our control over uwsgi to load/execute it. We can interact with uwsgi using magic variables. More on this can be found in the uwsgi docs [here](https://uwsgi-docs.readthedocs.io/en/latest/Vars.html). 
<br>
