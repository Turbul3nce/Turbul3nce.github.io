...
##### Challenge: 
The name and description pretty much gave away the vulnerability. However, it did require reading the source code and scripting to generate the possible reset tokens needed to brute-force the password reset. I spent a bit more time on this than I should have at first because I was confusing milliseconds with seconds 1o1 (basically not generating enough tokens.
...
<br>
After reviewing the source code, we can see the password reset tokens are generated by concatenating the user's email with the current Unix timestamp, then hashing the result. To exploit this, I first wrote a script to generate a token for my test user, test@hackthebox.com, using the same logic as the application. However, I eventually realized this approach wouldn't work since even a slight time difference—down to milliseconds—would result in a different token, making it nearly impossible to match the token precisely.
<br>
